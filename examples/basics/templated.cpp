...
int main( int argc, char* argv[] )
{
  if( argc != 2 )
  {
    std::cout << "This test requieres 1 parameter: NumResizedNodesInPython."
              << std::endl;
    return 1;
  }
  else
  {
    numResizedNodesInPython = atoi( argv[1] );
  }
  try
  {
    //BPCode
    PyGEmSManager myPyGEmSManager
      ( "StrFramework", &initTStrFramework, "Strategies", "Strategies.py" );
    ...
    std::cout
      ...
      << std::endl;
    for( int i = 0; i < vecSize; ++i )
      std::cout << myTStrategy.getContainer( )[i].stringParam << std::endl;
